...
LLM-based program repair has moved from single-turn toward agentic frameworks that allow the model to iteratively gather context and revise its own output. (cite)

AutoPatch constructs a benchmark of LLM-augmented CVE variants for vulnerability detection and patching, leveraging RAG-DB-assisted prompting \cite{seo2026autopatch, Seo2025AutoPatchMF}.
75 high-severity, real-world CVEs
using five popular LLMs to evaluate our system. Among
them, GPT-4o shows the best performance, achieving an F1-
score of 90.3% in vulnerability verification and 94.1% in
patching, particularly excelling in concurrency-related issues
and validation and logic handling issues. Compared to traditional fine-tuning approaches, AutoPatch is significantly
...
context-aware verification and patching mechanism is particularly effective at surfacing semantic inconsistencies, especially
in validation and logic-related vulnerabilities
...
and 10 for o3-mini, these cases are uncommon
% the Dataset chapter covers what you actually found when you used it (surface-level augmentations invisible at CPG level).
This thesis uses AutoPatch as one of its two evaluation datasets (see Chapter~\ref{chapter:dataset}). 
 
RepairAgent proposes bug fixes using an autonomous agent powered by Large Language Models. The agent plans and executes actions to fix bugs by invoking suitable tools, which include: gathering information about
...
RepairAgent imposes an average cost
