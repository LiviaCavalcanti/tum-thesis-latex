...
QLCoder is an agentic framework that automatically synthesizes CodeQL queries directly from CVE metadata, bridging the gap between natural-language vulnerability reports and actionable detection tools.
%TODO: restore/verify a citation key for QLCoder; add its BibTeX entry to references.bib once confirmed.
It embeds an LLM in a structured synthesis loop: the agent proposes a candidate query, a validator executes it against both the vulnerable and patched versions of a repository, and the resulting feedback is used to refine the next proposal. A query is only accepted once it satisfies \emph{MFix discrimination}, i.e., it is precise enough to flag the vulnerable version while remaining silent on the patched one. The system is evaluated on CWE-Bench-Java, comprising 176 CVEs across 111 Java projects and 42 CWE categories, such as Path Traversal, Cross-Site Scripting, and Code Injection, including 65 CVEs disclosed after the Claude Sonnet 4 training cutoff to reduce the risk of data leakage.
