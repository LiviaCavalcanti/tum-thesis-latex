...
Software vulnerabilities are often variations of flaws that have already been identified, disclosed, and fixed somewhere else~\cite{Kim2017VUDDY, Jang2012ReDeBug}.
Once a vulnerability is found and patched in one project, the same bug class may resurface in other projects that share a library, a coding pattern, or simply a common ancestor in the open-source dependency graph.
...
Standards like these establish best practices for data protection, cyber-resilience, and operational excellence.
To satisfy it and improve the robustness of development, enterprises must anticipate and adapt to vulnerabilities and potential attacks.
...
Under this effect, which prior work terms Analysis Focus Drift~\cite{contextenhancedYang}, the model ends up performing a generic code-quality review rather than a targeted vulnerability analysis.
...
Retrieval-augmented approaches are a natural fit for this gap, as they supply an LLM with structurally similar, previously fixed vulnerabilities as grounding evidence rather than asking it to reason about a vulnerability from generic pretraining knowledge.
...
The proposed pipeline only works once a vulnerability has been identified and its advisory has been created; therefore, it does not detect zero-days.
Instead, its goal is to maximize the propagation of that human knowledge, free security experts' time to find genuinely new vulnerabilities, reduce the window of exposure for already-known ones, and let teams without dedicated security expertise keep their codebases free of previously cataloged flaws.

% is it really necessary?
Concretely, the system assumes that the vulnerable function to be patched and its CWE category are already known and provided as input; detecting and localizing a vulnerability are outside the scope of this thesis. The retriever only needs the function's code, not its CWE, to find a structurally similar historical fix. 
